...
class RadarMessagingService : FirebaseMessagingService() {
    ...
    override fun onMessageReceived(remoteMessage: RemoteMessage) {
        ...
        // A copied token alone must never trigger location collection.
        val accepted = runBlocking(Dispatchers.IO) {
            try {
                val uid = RadarSession.uid()
                val senderUid = remoteMessage.data["senderUid"]
                senderUid != null && remoteMessage.data["targetUid"] == uid &&
                    senderUid == RadarSession.approvedPartner()
        ...
        runBlocking(Dispatchers.IO) {
            try {
                ...
                if (lastLoc != null) {
                    ...
                    uploadLocation(appContext, ownerUid, lastLoc, elapsed = { elapsed() })
                } else {
                ...
                if (location != null) {
                    ...
                    uploadLocation(appContext, ownerUid, location, elapsed = { elapsed() })
    ...
    private suspend fun uploadLocation(
        ...
        Log.d("RadarService", "[${elapsed()}] Writing to Firestore: locations/$ownerUid")
        db.collection("locationsV2")
            .document(ownerUid)
            .set(payload, SetOptions.merge())
            .await()
        Log.d("RadarService", "[${elapsed()}] Firestore write confirmed for locations/$ownerUid")
    }

    override fun onNewToken(token: String) {
        super.onNewToken(token)
        runBlocking(Dispatchers.IO) {
            try { RadarSession.saveToken(token) }
            catch (e: Exception) { Log.w("RadarService", "Token registration will retry on app startup", e) }
        }
    }
